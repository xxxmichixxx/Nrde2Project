...
```bash
#cd /tungstenfs/nobackup/gbuehler/michi/Projects/Nrde2/BCLIP/experiment3/longreads
cd /tungstenfs/nobackup/gbuehler/michi/Projects/Nrde2/BCLIP/experiment4

module load Bowtie2
for f in *_noComp_noLCstrip.prinseq.fasta ; do ( nice nohup bowtie2 --sensitive -f -x /tungstenfs/scratch/gbuehler/michi/Annotations/Bowtie2/snRNA_consensus2/snRNA_consensus.fa -S ${f%_noComp_noLCstrip.prinseq.fasta}_snRNAv2.sam -U $f -p 20 ) ; done
module purge
```
...
for f in *_snRNAv2_20ntOrLongerPerfectMatches.sam ; do ( (samtools view -b $f > ${f%.sam}.bam ; samtools sort ${f%.sam}.bam > ${f%.sam}-sorted.bam ; samtools index ${f%.sam}-sorted.bam & ) & ) ; done
```

Count depth:

```bash
for f in *_snRNAv2_20ntOrLongerPerfectMatches-sorted.bam ; do ( nohup samtools depth -d 1000000000 -a $f > ${f%_20ntOrLongerPerfectMatches-sorted.bam}_snRNAv2_pileup.txt & ) ; done
```
...
```{bash}
for f in *_noComp_noLCstrip.prinseq.fasta ; do ( wc -l $f > ${f%_noComp_noLCstrip.prinseq.fasta}_reads.txt ); done
```
...
files <- list.files("/tungstenfs/nobackup/gbuehler/michi/Projects/Nrde2/BCLIP/experiment3/longreads",full.names = TRUE, pattern="_snRNAv2_snRNAv2_pileup.txt")[1:12]
#files <- list.files("/tungstenfs/scratch/gbuehler/michi/Projects/Nrde2/BCLIP/experiment4",full.names = TRUE, pattern="snRNAv2_pileup.txt")
...
#filenames <- gsub("_snRNAv2_snRNAv2_pileup.txt","",files)
#filenames <- gsub("/tungstenfs/scratch/gbuehler/michi/Projects/Nrde2/BCLIP/experiment4/2743_BCLIP_","",filenames)
...
#read in read counts
read.count.files <- list.files("/tungstenfs/nobackup/gbuehler/michi/Projects/Nrde2/BCLIP/experiment3/longreads",full.names = TRUE, pattern="reads.txt")[1:12]
...
ggsave("/tungstenfs/nobackup/gbuehler/michi/Projects/Nrde2/Figures/snRNAv2_plots_BCLIP_exp3_cpm.pdf",device="pdf",height=8, width=16)

ggplot(D2,aes(x=position,y=cpm,col=sample)) + geom_line() +
   scale_color_manual(values=plotcols3) + theme_classic() + facet_wrap(vars(RNA),scales="free")

ggsave("/tungstenfs/nobackup/gbuehler/michi/Projects/Nrde2/Figures/snRNAv2_plots_BCLIP_exp3_freescales_cpm.pdf",device="pdf",height=8, width=16)
