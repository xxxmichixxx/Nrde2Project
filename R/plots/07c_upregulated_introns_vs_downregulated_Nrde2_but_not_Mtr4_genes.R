...
#res2 <- read.table(file="FigureData/CHX_Nrde2_Ccdc174_intronicRNAseq_DEseq2_results_0.05_0.5FC.txt",sep="\t",header=TRUE)
...
#----------------------------------------------------------------------------------------------------------------
#   # select genes downregulated in. Ntr4 but not Mtr4 (vs also mtr4) and compare upregulated introns within them
#-----------------------------------------------------------------------------------------------------------------
#group the genes into Mtr4 and Nrde2 regulated
Nrde2.up <- dplyr::filter(res1,padj < 0.01, log2FoldChange > 1 & Contrast== "Nrde2KO_vs_WT")
Nrde2.down <- dplyr::filter(res1,padj < 0.01, log2FoldChange < -1 & Contrast== "Nrde2KO_vs_WT")
Mtr4.up <- dplyr::filter(res1,padj < 0.01, log2FoldChange > 1 & Contrast== "Mtr4KO_vs_WT")
Mtr4.down <- dplyr::filter(res1,padj < 0.01, log2FoldChange < -1 & Contrast== "Mtr4KO_vs_WT")

res3 <- res2[res2$Contrast=="Nrde2KO_vs_WT",]


res3$NMreg <- ifelse(res3$GeneID %in% Nrde2.up$GeneID & res3$GeneID %in% Mtr4.up$GeneID, "Nrde2&Mtr4_up",
...
table(res3$intron_regulated)
table(res3$NMreg)
table(res3$NMreg[res3$intron_regulated=="up"])
table(res3$NMreg[res3$intron_regulated=="down"])
table(res3$NMreg[res3$intron_regulated=="no"])

unique(res3$GeneID[res3$intron_regulated=="up" & res3$NMreg == "Nrde2_down"])
unique(res3$gene_name[res3$intron_regulated=="up" & res3$NMreg == "Nrde2&Mtr4_down"])
unique(res3$gene_name[res3$intron_regulated=="up" & res3$NMreg == "Mtr4_down"])

res1[res1$gene_symbol=="Cdk2"&res1$Contrast=="Nrde2KO_vs_WT" ,]
res3[res3$GeneID=="ENSMUSG00000025358",]
...
length(unique( names(res3.gr))) == length( names(res3.gr))
  ...
  save(counts.donors.means,counts.RNA.means,counts.ribo.means,file="FigureData/heatmap_counts_at_5pSS_Nrde2KOvsWT_wholeUniqueReads_reps_combined_210331.RData")
  
 # load("FigureData/heatmap_counts_at_5pSS_Nrde2KOvsWT_CHX_wholeUniqueReads_reps_combined_210309.RData")
  ...
  ggsave("Figures/metaplots_at_Nrde2KO_vs_WT_CHX_introns_upregulated_5p-splice_junctions_wholeUniqueReads_reps_combined_Nrde2_data.pdf",device="pdf",height=5,width=12)
  ...
  ggsave("Figures/metaplots_at_Ccdc174_vs_WT_CHX_introns_upregulated_5p-splice_junctions_wholeUniqueReads_reps_combined_Ccdc174data.pdf",device="pdf",height=5,width=12)
